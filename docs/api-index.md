...
| `GET desktop-mode/v1/network/identity` (public), `GET desktop-mode/v1/network` (signed by a pinned member, or an administrator), `POST desktop-mode/v1/network/hop` (a logged-in user, towards a switcher entry) | [`network.md` → Developer surface](./network.md#developer-surface) | Experimental |
