...
const setup = () => {
	const ctx = mockViewContext< UsersState, UsersData >( { root: document.createElement( 'div' ), state: { tab: 'roles' } as UsersState, data: { list: { items: [], total: 20, pages: 25, page: 1, perPage: 20 } } } );
	ctx.repaint = vi.fn();
	Object.assign( ctx, { fetch: vi.fn( async () => new Response( JSON.stringify( snapshot ) ) ) } );
	return { ctx, summary: new RolesSummary() };
};
describe( 'complete role snapshots', () => {
	it( 'uses one request independently of directory pagination and does not refetch on repaint', async () => {
		...
		expect( ctx.fetch ).toHaveBeenCalledWith( '/desktop-mode/v1/users/roles-summary', expect.objectContaining( { signal: expect.any( AbortSignal ) } ) );
	} );
