...
import { activityModel, contribution, contributors, type ContributionKind } from './activity-model';
...
export function activityView( rows: UserListItem[], actions: RowActions, cfg: ProfileConfig, kind: ContributionKind, pick: ( value: ContributionKind ) => void ): TemplateResult {
	const model = activityModel( rows );
	const ranked = contributors( rows, kind );
	const top = ranked[ 0 ];
	...
	return html`<div class="os-community">
		<div class="os-community__metrics" aria-label=${ __( 'Community overview' ) }>
			${ metric( number( model.online.length ), __( 'Online now' ), sprintf( /* translators: %d: away users. */ __( '%d away at last refresh' ), model.away.length ), 'dashicons-admin-users' ) }
		...
		<div class="os-community__grid">
			<section class="os-community__panel os-community__work">
				...
				<p class="os-community__footnote">${ __( 'All-time published work and approved comments.' ) }${ model.known < rows.length ? ` ${ sprintf( /* translators: %d: users with missing counts. */ __( 'Content counts are unavailable for %d profiles.' ), rows.length - model.known ) }` : '' }</p>
			</section>
			<section class="os-community__panel os-community__presence-panel">
				<header><div><span class="os-community__eyebrow">${ __( 'AT LAST REFRESH' ) }</span><h3>${ __( 'Around the station' ) }</h3></div><span class="os-community__status-dot" data-online=${ model.online.length > 0 } aria-hidden="true"></span></header>
				<div class="os-community__presence-summary"><strong>${ number( present.length ) }</strong><p>${ __( 'online or away' ) }<span>${ sprintf( /* translators: 1: online users, 2: away users. */ __( '%1$d online · %2$d away' ), model.online.length, model.away.length ) }</span></p></div>
				...
				<p class="os-community__footnote">${ present.length > 6 ? sprintf( /* translators: %d: additional present users. */ __( 'And %d more across this site.' ), present.length - 6 ) : '' }${ __( 'Presence is a snapshot, refreshed with this view.' ) }${ model.unknownPresence ? ` ${ sprintf( /* translators: %d: users missing presence. */ __( '%d profiles have no presence data.' ), model.unknownPresence ) }` : '' }</p>
			</section>
